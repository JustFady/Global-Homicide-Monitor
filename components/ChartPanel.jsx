...
export default function ChartPanel({ countries, cities, onClose, onPin }) {
    ...
    return (
      <div className="rounded-xl glass-strong shadow-glow p-3 max-w-[260px] text-xs">
        ...
        <div className="mt-2 space-y-0.5 text-slate-400">
          ...
          <div>Under 25: {c.underAge25Percent.toFixed(0)}%</div>
        </div>
        <button
          onClick={() => onPin(c.id)}
          className="mt-2 w-full rounded-lg bg-accent/15 border border-accent/25 px-2 py-1.5 text-xs text-accent hover:bg-accent/20 transition"
        >
          Pin to Compare
        </button>
      </div>
    ...
    return (
      <div>
        ...
        <select
          ...
          className="w-full bg-white/5 border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-slate-300"
        >
  ...
  return (
    <div className="w-full h-full rounded-2xl glass shadow-glow overflow-hidden flex flex-col animate-slide-up">
      {/* Header */}
      ...
      {/* Body */}
      <div className="flex-1 overflow-auto p-4 lg:grid lg:grid-cols-[220px_1fr] gap-4">
        {/* Filters */}
        ...
        <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 flex flex-col">
          <div className="flex items-center justify-between gap-3 mb-3 flex-shrink-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium text-slate-400">Rank by</span>
              <select
                value={metricKey}
                onChange={(e) => setMetricKey(e.target.value)}
                className="bg-white/5 border border-white/[0.08] rounded-lg px-2.5 py-1.5 text-xs text-slate-300"
              >
            ...
            <button
              ...
              className="rounded-lg bg-white/5 border border-white/[0.08] px-2.5 py-1.5 text-xs hover:bg-white/10 transition text-slate-400"
            >
          ...
          <div className="text-[11px] text-slate-600 mt-2">
            Top {chartData.length} results · {metricLabel}
          </div>
        </div>
      </div>
    </div>
